...
def test_metadata_outside_root_rejected(monkeypatch, tmp_path):
    from exif_tagger.db import get_connection, init_db
    gallery = tmp_path / "gallery"
    gallery.mkdir()
    secret = tmp_path / "secret.txt"
    ...
    cfg.write_text(f"root_directory: {gallery}\nmodel:\n  base_url: http://x/v1\n  model_name: m\n")
    monkeypatch.setattr(server_module, "CONFIG_PATH", str(cfg))
    resp = _client().get(f"/api/gallery/image/{image_id}", headers=HEADERS)
    assert resp.status_code == 403


def test_by_path_absolute_outside_root_rejected(monkeypatch, tmp_path):
    gallery = tmp_path / "gallery"
    gallery.mkdir()
    cfg = tmp_path / "config.yaml"
    cfg.write_text(f"root_directory: {gallery}\nmodel:\n  base_url: http://x/v1\n  model_name: m\n")
    monkeypatch.setattr(server_module, "CONFIG_PATH", str(cfg))
    resp = _client().get("/api/gallery/image/file", params={"path": "/etc/passwd"}, headers=HEADERS)
    assert resp.status_code == 403
